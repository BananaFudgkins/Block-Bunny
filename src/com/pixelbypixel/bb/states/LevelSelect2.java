...
public class LevelSelect2 extends GameState {
	...
	public LevelSelect2(GameStateManager gsm) {
		...
		for(int row = 0; row < buttons.length; row++) {
			for(int col = 0; col < buttons[0].length; col++) {
				...
				buttons[row][col].setText(row * buttons[0].length + col + 26 + "");
			}
		}
	...
	public void handleInput() {
		if(leftarrowButton.isClicked()) gsm.setState(GameStateManager.LEVEL_SELECT);
	}
		...
		for(int row = 0; row < buttons.length; row++) {
			for(int col = 0; col < buttons[0].length; col++) {
				...
				if(buttons[row][col].isClicked()) {
					Play.level = row * buttons[0].length + col + 26;
